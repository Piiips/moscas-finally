...
nav = [img.load('N1.jpg'),img.load('N2.jpg'),img.load('N3.jpg'),img.load('N4.jpg'),img.load('N5.jpg'),img.load('N6.jpg'),img.load('N7.jpg'),img.load('N8.jpg'), img.load('N7.jpg'),img.load('N6.jpg'),img.load('N5.jpg'),img.load('N4.jpg'),img.load('N3.jpg')]
...
class asteroid(object):
    ...
    def colision(self, rect):
        ...
        if rect[1] - rect[3] < self.hitbox[1] and rect[0] < self.hitbox[0] + self.hitbox[2] and rect[0] > self.hitbox[0]:
            return True
        return False
...
def redibujarpantalla():
    ...
    if len(planetas) == 1 :
        if contador == 0:
            win.blit(planetas[0], (540, tierra_y))
    ship.draw(win)
    for j in asteroides:
        if j == asteroide:
            asteroide.draw(win)
...
class juego():
    ...
    def correr(run):
        ...
        while run:
            ...
            bgY2 += 5
            tierra_y += 5
            asteroide.y, asteroide2.y = bgY, bgY
            ...
            if bgY > bg.get_height():
                ...
                asteroide.x, asteroide2.x = random.randint(0, 690-asteroide.width), random.randint(690, 1380-asteroide.width)
            ...
            if keys[pygame.K_SPACE] and blastloop == 0:
                if len(blaster) < 15:
                    ...
                    blaster.append(projectile(round(ship.x - 27 + ship.width // 2), round(ship.y + ship.height//3), 6, (255, 0, 0), 3, 25))
                    blaster2.append(projectile(round(ship.x + 34 + ship.width // 2), round(ship.y + ship.height // 3), 6, (255, 0, 0), 3,25))
                    blastloop += 1
            ...
            if keys[pygame.K_a] and ship.x > 0:
                ...
                ship.left = True
                ship.right = False
            elif keys[pygame.K_d] and ship.x < 1380-ship.width:
                ship.x += ship.vel
                ship.left = False
                ship.right = True
...
#de momento hay solamente asteroides que aparecen cada dos pantallas, despues se le añadiran naves que sirvan de enemigo que.
#tambien dispare y nos pueda hacer daño por lo que implementare un sistema de barras de vida.
#los cuadros de hitbox tambien se corregiran y seran invisibles
